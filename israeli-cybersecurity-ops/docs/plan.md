# Israeli Cybersecurity Ops Skill — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build a skill that teaches Claude to coordinate Israeli-built cybersecurity tools (Wiz, Snyk, Check Point, CyberArk, SentinelOne) for security operations workflows — threat triage, vulnerability management, and compliance checking.

**Architecture:** Multi-MCP Coordination skill (Pattern 2). Orchestrates across multiple Israeli cybersecurity platforms, embedding best practices for incident triage, vulnerability prioritization, and security posture assessment.

**Tech Stack:** SKILL.md, references for each vendor's API, security operation playbooks.

---

## Research

### Israeli Cybersecurity Companies with MCP/API Access

**Already have MCP servers:**
- **Wiz:** Active MCP server used in production (Grammarly case study)
  - Cloud security posture, vulnerability findings, compliance
- **Snyk:** Official MCP server in Snyk CLI v1.1296.2+
  - 11+ tools: SAST, SCA, IaC scanning, container scanning
- **Monday.com:** MCP server (for tracking security tasks)

**Have APIs, no MCP yet:**
- **Check Point:** SmartConsole API, Management API, Threat Prevention API
  - Community MCP server may exist
  - Endpoints: policy management, threat logs, IoC checking
- **CyberArk:** REST API for Privileged Access Management
  - Identity Flows for automation
  - Credential retrieval, session management
- **SentinelOne:** RESTful API with Singularity Marketplace
  - 300+ integrations
  - Threat detection, EDR, incident response
- **Armis:** RESTful API
  - Asset inventory, vulnerability management
  - IoT/OT security
- **Torq:** 500+ integrations, essentially an automation platform
  - SOC automation, playbook execution
- **Pentera:** REST API for automated penetration testing

### Security Operations Workflows
1. **Tier-1 Incident Triage:** Alert -> Enrich -> Classify -> Escalate/Close
2. **Vulnerability Prioritization:** Scan -> CVSS + Context -> Business Impact -> Remediation Plan
3. **Compliance Check:** Framework selection -> Control mapping -> Gap assessment -> Report
4. **Threat Hunt:** Hypothesis -> Data collection -> Analysis -> Findings

### Use Cases
1. **Multi-tool security triage** — Correlate alerts from Wiz (cloud), Snyk (code), SentinelOne (endpoint)
2. **Vulnerability prioritization** — Combine scan results with business context
3. **Security posture assessment** — Check compliance against frameworks (SOC2, ISO27001, Israeli privacy law)
4. **Incident response playbook** — Step-by-step guided response using available tools
5. **Israeli-specific compliance** — Map Israeli Privacy Protection Law and INCD guidelines to controls
...
**Files:**
- Create: `repos/security-compliance/israeli-cybersecurity-ops/SKILL.md`
...
- `references/tool-api-reference.md` — API endpoints for each Israeli cybersecurity tool
- `references/israeli-compliance-frameworks.md` — INCD guidelines, Privacy Law controls, Banking Supervision
- `references/triage-playbooks.md` — Step-by-step playbooks for common incident types
