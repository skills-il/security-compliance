...
4. SOC2 / ISO27001 (international); MITRE ATT&CK is at **v19.2** (released 2026-08-05; the v19 line began 2026-04-28). Detection Strategies and Analytics arrived in v18 (2025) and carry forward. Check the current tag before citing a version, this moves several times a year
...
Actions: Follow Workflow C, map Israeli Privacy Protection Law requirements as amended by Amendment 13, determine whether a DPO appointment is triggered, check whether the database is registrable at all under the new regime (most are not), review consent mechanisms, assess cross-border data flows.

## Bundled Resources

### Scripts
- `scripts/security_triage.py`, Structured security alert triage tool that calculates composite severity scores from CVSS, asset criticality, data sensitivity, and blast radius. Determines INCD reporting obligations for critical infrastructure and Privacy Authority notification for data breaches. Outputs classification, recommended response steps, and reporting deadlines. Run: `python3 scripts/security_triage.py --help`. Inputs are validated: a CVSS outside 0.0-10.0 is clamped with a warning on stderr, and an unrecognised `--asset` / `--data` / `--blast` value is scored as a neutral 5 with a warning naming the valid values. `--asset`/`--data`/`--blast` are a closed set and an invalid value now fails rather than scoring a silent default. Pass `"sector"` in the JSON payload (`banking`, `finance`, `insurance`, `pension`, `health`, `hmo`, `hospital`) so sector-regulator reporting is assessed; omit it and the tool says so rather than implying no report is owed. When CVSS is 0 or absent the tool drops that dimension and renormalises the remaining weights, because ransomware and insider exfiltration have no CVSS and a fixed CVSS weight capped them at 7.0, below the CRITICAL threshold.
...
- **Three reporting duties run independently, and satisfying one does not satisfy another.** A report to CERT-IL is incident-response assistance, not a regulatory filing. It does not discharge the Privacy Protection Authority duty, and neither discharges a sector regulator's duty (Supervisor of Banks under Directive 364, the Capital Market Insurance and Savings Authority, the Ministry of Health). A supervised bank or HMO is frequently NOT INCD-designated critical infrastructure, so "INCD reporting: not required" must never be read as "no report is owed". Always establish which regulator supervises the entity.
- **Paying a ransom is a legal decision, not a SOC decision.** For an Israeli entity a payment to an entity linked to a designated terrorist organisation can create liability under the Counter-Terrorism Law 2016 and the Prohibition on Money Laundering Law, on top of foreign sanctions exposure. Never present payment as an operational option. Route it to counsel and engage INCD/CERT-IL.
- **Preserve before you contain.** Isolating a host destroys volatile memory. For a live intrusion, capture memory first, then contain. Both the PPA and the INCD may later ask for the evidence.
- **The triage score is an input to a judgement, not the judgement.** `security_triage.py` scores a typo'd category as a neutral 5 rather than erroring, so a misspelled `--asset prodution` silently downgrades a production incident. The script warns on stderr; an agent that captures only stdout will not see it. Always echo the `cvss_used` and category values back to the user alongside the score.
